...
import { COHERE_API_KEY, COHERE_ENDPOINT, GROK_API_KEY, GROK_ENDPOINT, OPENROUTER_API_KEY, OPENROUTER_ENDPOINT, GOOGLE_API_KEY, GOOGLE_ENDPOINT } from "@/lib/api-config";
...
let useGrokAPI = false; // Desactivamos el uso de la API de Grok por defecto
let useGoogleAPI = true; // Activamos el uso de la API de Google por defecto
...
export const setUseGrokAPI = (useIt: boolean) => {
  useGrokAPI = useIt;
  // Si activamos Grok, desactivamos Google para evitar conflictos
  if (useIt) {
    useGoogleAPI = false;
  }
};
...
export const setUseGoogleAPI = (useIt: boolean) => {
  useGoogleAPI = useIt;
  // Si activamos Google, desactivamos Grok para evitar conflictos
  if (useIt) {
    useGrokAPI = false;
  }
};

export const getUseGoogleAPI = () => {
  return useGoogleAPI;
};
...
export const callGoogleGeminiAPI = async (prompt: string): Promise<string> => {
  ...
  try {
    ...
    const endpoint = GOOGLE_ENDPOINT || "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent";
    
    const apiKeyParam = `?key=${GOOGLE_API_KEY}`;
    const fullEndpoint = `${endpoint}${apiKeyParam}`;
...
export const callGeminiAPI = async (prompt: string): Promise<string> => {
  // Si está activado el uso de Google Gemini, usar esa API en su lugar
  if (useGoogleAPI && GOOGLE_API_KEY) {
